...
class DietUserView(LoginRequiredMixin, TemplateView):
    """Widok, który renderuje dietę użytkownika."""
...
def main_page_redirect(request):  # noqa: D103
...
def diet_list(request, nut_id):
    """Widok, który renderuje listę diet."""
    try:
...
def diet_creator(request, diet_id):  # noqa: D103
    """Widok, który renderuje edytor diety."""
...
def diet_creator_first(request):
    """Widok, który renderuje edytor tworzenia diety."""
    if request.method == 'GET':
    ...
    return render(
        request,
        'diet_creator_first.html',
        {
            'form_diet': form_diet,
            'nut_id': request.user.nutritionist.id,
        },
    )


def diet_days_list(request, diet_id):
    """Widok, który renderuje listę dni danej dity."""
...
def diet_day_creator(request, diet_id, diet_day_id):
    """Widok, który renderuje edytor dnia danej diety."""
...
def diet_day_creator_first(request, diet_id):
    """Widok, który renderuje edytor tworzenia dnia danej diety."""
    diet = Diet.objects.get(id=diet_id)
    if request.method == 'GET':
...
def products_list(request, diet_day_id, diet_id):
    """Widok, który renderuje listę produktów danego dnia diety."""
    products = Product.objects.filter(diet_day_id=diet_day_id)
    return render(request, 'products_list.html',
                  {'products': products, 'diet_day_id': diet_day_id, 'diet_id': diet_id})


class ConsultationsListView(LoginRequiredMixin, ListView):
    """Widok, który generuję listę konsultacji."""
...
class AcceptedConsultationsListView(LoginRequiredMixin, ListView):
    """Widok, który renderuje zaakceptowane konsultacje."""
...
def product_creator(request, diet_day_id, product_id):
    """Widok, który renderuje edytor produktów."""
...
def product_creator_first(request, diet_day_id):
    """Widok, który renderuje edytor dodawania produktów."""
    diet_day = DietDay.objects.get(id=diet_day_id)
    if request.method == 'GET':
    ...
    if request.method == 'POST':
        ...
        if form_product.is_valid():
            form_product.save()
            return redirect('diet:products_list', diet_day_id, diet_day.diet.id)
    return render(request, 'product_creator_first.html',
                  {
                      'form_product': form_product,
                      'diet_day_id': diet_day_id,
                      'diet_id': diet_day.diet.id,
                  },
                  )


def nutrients_creator(request, product_id, diet_day_id):
    """Widok, który renderuje edytor składników odżywczych."""
    product = Product.objects.get(id=product_id)
    ...
    return render(
        request,
        'nutrients_creator.html',
        {
            'form_nutrients': form_nutrients,
            'diet_day_id': diet_day_id,
            'diet_id': product.diet_day.diet.id,
        },
    )


class ProductDetailView(TemplateView):
    """Widok, który renderuje szczegóły produktu."""
